...
def predict(news_content):
    ...
    model = BertForSequenceClassification.from_pretrained(r'train_model\bert_trained_model', ignore_mismatched_sizes=True)
    tokenizer = BertTokenizer.from_pretrained(r'train_model\bert_trained_model')
    ...
    #     return pred.item()  # Return predicted class (0 or 1)
    # possibility
    with torch.no_grad():
        outputs = model(**inputs)
        logits = outputs.logits
